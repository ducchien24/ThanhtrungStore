...
const ProductDetail = (props: any) => {
  ...
  const fetchListProduct = async () => {
    try {
      ...
      if (res?.code === SUCCESS_CODE) {
        setDataProduct(res);
        console.log("product detail info: ", res)
  ...
  React.useEffect(() => {
    if (width) {
      setSizeImage(width / 3 - 20);
      setSizeImageSmall(width / 15 - 14);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [width]);
                  ...
                  <Box>
                    <Row>
                      ...
                      <IconButton
                        aria-label="add"
                        onClick={() => {
                          let listProductInCart = JSONbig.parse(localStorage.getItem(CART_LOCAL_STORAGE)|| ``) ;
                          
                          const productInfoInCart = listProductInCart.find((product:any)=>product.id===dataProduct.message.id)
                          console.log("count: ", count, "Quanlity: ", productInfoInCart)
                          if(productInfoInCart===undefined){
                            if(count <= dataProduct.message.quanlity){
                                       setCount(count+1); 
                                                 }
                            else{
                          ...
                          else if(count + productInfoInCart.count < dataProduct.message.quanlity){
                            setCount(count + 1);
